...
module.exports = React.createClass({
    ...
    render: function render() {
        ...
        var cols = _.map(_.without(_.keys(this.props.data[0]), _.map(columnMeta, function (i) {
            return i.columnName;
        })), function (item) {
        ...
        return React.createElement(Griddle, {
            ...
            showFilter: true,
            resultsPerPage: 20,
            columns: ['institution', 'collection', 'contact', 'contact_role', 'update_url', 'collection_uuid'],
...
var LinkName = React.createClass({
    ...
    render: function render() {
        ...
        if (d == null || d.toLowerCase() == 'na') {
            ...
            return React.createElement(
                'a',
                { href: href, target: "_collection_" + this.props.rowData.collection_uuid },
                d
            );
        }
    }
            ...
            records.forEach(function (item) {
                if (item.trim().length > 0) {
                    links.push(React.createElement(
                        'a',
                        { href: '/portal/recordsets/' + item.trim(), key: item },
                        item
                    ));
                }
            });
...
var MapLink = React.createClass({
    ...
    render: function render() {
        ...
        if (_.isNull(this.props.rowData.lat) || _.isNull(this.props.rowData.lon)) {
            ...
            return React.createElement(
                'a',
                { href: d, target: "_new", onClick: this.openClick },
                'Show On Map'
            );
        }
    }
