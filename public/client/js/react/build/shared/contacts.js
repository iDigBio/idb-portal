...
var defOrNone = function defOrNone(obj, key) {
    if (_.isArray(key)) {
        var str = '';
...
module.exports = React.createClass({
    ...
    makeContact: function makeContact(contact) {
        var name = defOrNone(contact, ['first_name', 'last_name']);
        var email = defOrNone(contact, 'email');
        email = _.isString(email) ? React.createElement(
            'a',
            { href: 'mailto: ' + email },
            email
        ) : email;
    ...
    render: function render() {
        if (_.has(this.props.data, 'contacts') && _.isArray(this.props.data.contacts)) {
            ...
            return React.createElement(
                'div',
                { id: "contacts", className: "clearfix scrollspy" },
                React.createElement(
                    'h2',
                    { className: "title" },
                    'Contacts'
