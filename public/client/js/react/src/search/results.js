...
var ResultsLabels = React.createClass({
    ...
    makeLabel: function(result,id){
        ...
        if( data.hasImage ){ 
            ...
            img = (
                <span key={'media-'+result.uuid+this.props.stamp} className="image-wrapper" id={data.mediarecords[0]} onClick={this.openMedia} title="click to open media record">
                    {imgcount}
                    <img data-onerror="$(this).attr('src','/portal/img/notavailable.png')" data-onload="$(this).attr('alt','image thumbnail')" className="pull-right label-image" alt={title} src={"https://api.idigbio.org/v1/records/"+result.uuid+"/media?quality=thumbnail"} /> 
                </span>  
            )
