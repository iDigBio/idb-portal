...
module.exports = function(elid, options) {
    ...
    ****/
    var resizeFunction = function() {
        ...
        $('#' + elid)
        .css('width', (width - 53) + 'px')
        .css('height', (height - 53) + 'px');

        $('#' + elid).css('width', (width - 53) + 'px')
        .css('height', (height - 53) + 'px');
    ...
    var ImageButton = L.Control.extend({
        ...
        imageClick: function(map) {
            return function(e) {
                e.preventDefault();
                e.stopPropagation();
                $('#map-image-button').removeClass('camera-icon')
                .addClass('spinner');
                ...
                leafletImage(map, function(err, canvas) {
                    // build white box
                    var context = canvas.getContext('2d'), height = canvas.height;
                    ...
                    legend.order.reverse().forEach(function(item, index) {
                        context.beginPath();
    ...
    var mapClick = function(e) {
        ...
        getPoints(0, function(data) {
            if(data.itemCount > 0) {
                ...
                $('.nav-left, .nav-right').off('click', navClick);
                $('.set-bounds-link').off('click', setClick);
                popup.setLatLng(e.latlng).setContent(makeContent(data, 0))
                .openOn(self.map);
    ...
    this.query = function(query) {
        ...
        if(_.has(query, 'geopoint')) {
            if(query.geopoint.type === 'geo_distance') {
                ...
                var gg = query.geopoint;
                this.map.fitBounds([[gg.top_left.lat, gg.top_left.lon], [gg.bottom_right.lat, gg.bottom_right.lon]]);
            }
        } else {
