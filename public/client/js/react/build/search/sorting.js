...
var Sort = module.exports = React.createClass({
    ...
    //mixins: [PureRender],
    statics: {
        defaultSorts: function defaultSorts() {
            return [{ name: 'genus', order: 'asc' }, { name: 'specificepithet', order: 'asc' }, { name: 'datecollected', order: 'asc' }];
        }
    },
    getSortNames: function getSortNames() {
        var list = [];
    ...
    scrollSorts: function scrollSorts(e) {
        e.preventDefault();
        $('#sort-group').animate({
    ...
    render: function render() {
        ...
        this.props.sorting.forEach(function (item, ind) {
            ...
            _.each(groups, function (val) {
                ...
                _.each(fields.byGroup[val], function (field) {
                    if (field.hidden === 1) {
                        //noop
                            ...
                            flist.push(React.createElement(
                                'option',
                ...
                fgroups.push(React.createElement(
                    'optgroup',
                    { label: fields.groupNames[val], key: val + ind },
                    '  ',
                    flist
                ));
            });
            ...
            if (ind === 0) {
                sorts.push(React.createElement(
                    'div',
                    { className: "option-group", key: ind },
                    React.createElement(
                        'label',
                        null,
                        'Sort by'
                    ...
                    React.createElement(
                        'button',
                        { onClick: self.removeClick, 'data-index': ind },
                        React.createElement('i', { className: "glyphicon glyphicon-minus" })
                    ),
                    React.createElement(
                        'select',
                        { className: "name form-control", value: item.name, onChange: self.sortChange, 'data-index': ind, 'data-name': 'name' },
                        fgroups
                    ),
                    React.createElement(
                        'select',
                        { className: "direction form-control", value: item.order, onChange: self.sortChange, 'data-index': ind, 'data-name': 'order' },
                        React.createElement(
                            'option',
                            { value: "asc" },
                            'Ascending'
        ...
        return React.createElement(
            'div',
            { className: "clearfix section " + this.props.active, id: "sorting" },
            React.createElement(
                'div',
                { id: "sort-add" },
                'Add a sort  ',
                React.createElement(
                    'button',
                    { onClick: this.addClick, disabled: disabled },
