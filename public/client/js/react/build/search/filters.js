...
var Filters = module.exports = React.createClass({
    ...
    mixins: [PureRender],
    statics: {
        newFilterProps: function newFilterProps(term) {
    ...
    filterPropsChange: function filterPropsChange(filterObj) {
    ...
    makeFilter: function makeFilter(filter) {
        //var type = fltrObj.type, name = fltrObj.name;
        //var type = 'text';
        var key = filter.name; // + Date.now();
        switch (filter.type) {
            case 'text':
                return React.createElement(TextFilter, { key: key, filter: filter, removeFilter: this.removeFilter, search: this.props.search, changeFilter: this.filterPropsChange });
            case 'daterange':
                return React.createElement(DateRangeFilter, { key: key, filter: filter, removeFilter: this.removeFilter, changeFilter: this.filterPropsChange });
            case 'numericrange':
                return React.createElement(NumericRangeFilter, { key: key, filter: filter, removeFilter: this.removeFilter, changeFilter: this.filterPropsChange });
        }
    },
    ...
    scrollFilters: function scrollFilters() {
    ...
    render: function render() {
            ...
            _.each(fields.byGroup[val], function (field) {
                if (field.hidden) {
                    //noop
                        ...
                        fltrs.push(React.createElement(
                            'option',
                            { disabled: disabled, value: field.term, key: field.term },
                            field.name
                        ));
                    }
            });
            fgroups.push(React.createElement(
                'optgroup',
                { key: val, label: fields.groupNames[val] },
                '  ',
                fltrs
            ));
        });
        ...
        return React.createElement(
            'div',
            { className: "section " + this.props.active, id: "filters" },
            React.createElement(
                'div',
                { className: "option-group", id: "filter-select" },
                React.createElement(
                    'select',
                    { className: "form-control", value: "0", placeholder: "select to add", onChange: this.addFilter },
                    React.createElement(
                        'option',
                        { value: "0", defaultValue: true, className: "default" },
                        'Add a field'
                    ),
                    fgroups
                ),
                React.createElement(
                    'a',
                    { className: "btn", onClick: this.clearFilters, title: "Clear all filter inputs" },
                    'Clear'
                )
            ),
            React.createElement(
                'div',
                { id: "filters-holder", className: "options-holder" },
                filters
            ),
            React.createElement(
                'div',
                { id: "filter-scroller", onClick: this.scrollFilters },
                React.createElement(
                    'span',
                    { style: { 'display': scrollDisplay } },
                    '↓ Scroll To Bottom ↓'
                )
            )
        );
...
var TextFilter = React.createClass({
    ...
    presenceClick: function presenceClick(event) {
    ...
    textType: function textType(event) {
    ...
    setAutocomplete: function setAutocomplete(event) {
        var self = this,
            name;
        var options = {
            source: function source(searchString, respCallback) {
    ...
    getSynonyms: function getSynonyms(event) {
        event.preventDefault();
    ...
    render: function render() {
        ...
        var name = filter.name,
            label = fields.byTerm[name].name;
        var syn = React.createElement('span', null),
            cl = 'text';
        if (fields.byTerm[name].synonyms) {
            syn = React.createElement(
                'a',
                { onClick: this.getSynonyms },
                'Add EOL Synonyms'
            );
            cl += ' syn';
        }
        ...
        return React.createElement(
            'div',
            { className: "option-group filter", id: name + '-filter', key: name },
            React.createElement(
                'a',
                { className: "remove", href: "#", onClick: this.props.removeFilter, 'data-remove': name },
...
var DateRangeFilter = React.createClass({
    ...
    dateChange: function dateChange(event) {
    ...
    render: function render() {
        ...
        return React.createElement(
            ...
            React.createElement(
                'div',
                { className: "dates clearfix pull-right" },
                React.createElement(
                    'div',
                    { className: "pull-left" },
                    'Start:',
...
var NumericRangeFilter = React.createClass({
    displayName: 'NumericRangeFilter',
    ...
    render: function render() {
        ...
        var name = filter.name,
            label = fields.byTerm[name].name;
        exists = filter.exists, missing = filter.missing, disabled = false;
        if (exists || missing) {
            disabled = true;
        }
        return React.createElement(
